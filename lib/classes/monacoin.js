...
var Monacoin = /** @class */ (function () {
    ...
    Monacoin.prototype._computeFees = function (byteCounts, feeRate) {
        var estimateFees = new bignumber_js_1.default(byteCounts).times(feeRate).toString();
        return estimateFees;
    };
    /**
    ...
    Monacoin.prototype._selectUtxos = function (options) {
        ...
        for (var _i = 0, _a = options.utxos; _i < _a.length; _i++) {
            ...
            // おつりアドレスなしでfeeを計算
            estimateFees = this._computeFees(estimateTxBytes_1.default(inputs, outputs), options.feeRate);
            if (sumInput.eq(amount.plus(estimateFees))) {
                hasChange = false;
                break;
            }
            // 十分なインプットがあるにも関わらず、おつりが作れない場合は、僅かな手数料を犠牲にしておつりをなくすことで送金可能にする。※インプットを追加すると余計に手数料がかかる
            if (sumInput.gt(amount.plus(estimateFees)) &&
                sumInput.lt(amount.plus(estimateFees).plus(this.minOutValue))) {
                estimateFees = new bignumber_js_1.default(estimateFees)
