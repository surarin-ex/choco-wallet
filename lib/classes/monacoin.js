...
var Monacoin = /** @class */ (function () {
    function Monacoin(mnemonic, chain) {
        ...
        this.minOutValue = (Math.min(estimateTxBytes_1.default(inputs, { P2PKH: 1 }), estimateTxBytes_1.default(inputs, { P2SH: 1 }), estimateTxBytes_1.default(inputs, { P2WPKH: 1 }), estimateTxBytes_1.default(inputs, { P2WSH: 1 })) * this.minFeeRate).toString(); // watanabe
        this._chain = chain;
        this._coin = "Monacoin";
     ...
     * Mnemonicを取得する
     */
    Monacoin.prototype.getMnemonic = function () {
     ...
     */
    Monacoin.prototype._computeFees = function (byteCounts, feeRate) {
        var fees = new bignumber_js_1.default(byteCounts).times(feeRate).toString();
        return fees;
    };
    /**
    ...
    Monacoin.prototype._selectUtxos = function (options) {
        if (options.algorithm === "Single Random Draw") {
            var result = singleRandomDraw_1.default({
                utxos: options.utxos,
                feeRate: options.feeRate,
                inputType: this.addressType,
                outputType: getOutputType_1.default(options.toAddress, this._network).toUpperCase(),
            ...
            var result = branchAndBound_1.default({
                utxos: options.utxos,
                feeRate: options.feeRate,
                inputType: this.addressType,
                outputType: getOutputType_1.default(options.toAddress, this._network).toUpperCase(),
    ...
    Monacoin.prototype.updateUnsignedTx = function (options) {
        return __awaiter(this, void 0, void 0, function () {
            var amount_big, allUtxos, _a, utxos, fees, hasChange, sumInput, psbt, inputDataList, outputDataList, outputData1, changeAmount, outputData2;
            var _this = this;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        amount_big = new bignumber_js_1.default(options.amount);
                        if (amount_big.lt(this.minOutValue) ||
                    ...
                    case 4:
                        ...
                        }), utxos = _a.utxos, fees = _a.fees, hasChange = _a.hasChange;
                        sumInput = getUtxosValue_1.default(utxos);
     ...
     * トランザクション履歴を更新するメソッド
     */
    Monacoin.prototype.updateHistory = function () {
        var _this = this;
        if (!this.txInfos || !this.addressInfos)
            return;
        var allAddresses = this.addressInfos.map(function (info) {
            return info.address;
        });
        ...
        var txHistories = [];
        this.txInfos.forEach(function (info) {
            var fromAddresses = flatten(info.vin.map(function (input) {
                return input.addresses;
            }));
            var toAddresses = flatten(info.vout.map(function (output) {
                return output.addresses;
            }));
            var hasMyInput = fromAddresses.some(function (address) {
                return allAddresses.includes(address);
            });
            var isMyInput = fromAddresses.every(function (address) {
                return allAddresses.includes(address);
            });
            var hasMyOutput = toAddresses.some(function (address) {
                return allAddresses.includes(address);
            });
            var isMyOutput = toAddresses.every(function (address) {
                return allAddresses.includes(address);
            });
            // 入金の場合
            if (!hasMyInput && hasMyOutput) {
                var value = "+" +
                    info.vout
                        ...
                        return output.addresses.every(function (address) {
                            return allAddresses.includes(address);
                        });
                    })
                ...
                txHistories.push({
                    ...
                    time: info.blockTime
                        ...
                        : "未承認",
                    type: "入金",
                    detailUri: "" + _this.blockbook.explorer + info.txid,
                    value: value,
                    fees: "0",
                    partner: "不明",
                    balance: "0",
            ...
            // 出金の場合
                ...
                var value = "-" +
                    new bignumber_js_1.default(info.value)
                        .minus(info.vout.reduce(function (acc, output) {
                        if (output.addresses.every(function (address) {
                            return allAddresses.includes(address);
                        })) {
                            return acc.plus(output.value);
                        }
                        ...
                        .dividedBy(_this.digit);
                var fees = "-" + new bignumber_js_1.default(info.fees).dividedBy(_this.digit).toString();
                txHistories.push({
                    txid: info.txid,
                    blockTime: info.blockTime,
                    time: info.blockTime
        ...
        // トランザクション履歴のソート(新しい順)
        txHistories.sort(function (a, b) {
            return b.blockTime - a.blockTime;
        });
        // balance計算
        var balance = new bignumber_js_1.default(0);
        for (var i = txHistories.length - 1; i >= 0; i--) {
